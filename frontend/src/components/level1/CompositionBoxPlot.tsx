...
interface BoxStats {
  ...
  outliers: number[];
  n: number;
  groupIdx: number; // color index based on primary group
}
...
function groupPatients(
  patients: Patient[],
  primary: GroupBy,
  secondary: GroupBy | null,
): BoxStats[] {
  // Build unique primary keys for color assignment
  ...
  return Object.entries(groups)
    .sort((a, b) => {
      // Sort by primary key first, then by secondary
...
function drawBoxPlot(canvas: HTMLCanvasElement, boxes: BoxStats[]) {
  ...
  boxes.forEach((b, i) => {
    ...
    ctx.moveTo(cx - capW, toY(b.whiskerHi)); ctx.lineTo(cx + capW, toY(b.whiskerHi));
    ctx.stroke();
    ...
    ctx.fillText(`n=${b.n}`, cx, h - padB + 28);
  });
...
interface CompositionBoxPlotProps {
  groupBy: GroupBy;
  splitBy: GroupBy | null;
}

export function CompositionBoxPlot({ groupBy, splitBy }: CompositionBoxPlotProps) {
  const patients = useDataStore((s) => s.patients);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);

  const boxes = useMemo(
    () => groupPatients(patients, groupBy, splitBy === groupBy ? null : splitBy),
    [patients, groupBy, splitBy],
  );
  ...
  return (
    <div className="h-full rounded-[1.25rem] border border-[var(--border)] bg-[var(--surface-overlay)]">
